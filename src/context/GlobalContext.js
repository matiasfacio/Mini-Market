  ...
  switch (action.type) {
    ...
    case "ADD_CART":
      return {
        ...
        Cart: [
          ...state.Cart,
          { ...state.ProductsAvailable[action.index], quantity: 1 },
        ],
        CountCart:
          state.CountCart + state.ProductsAvailable[action.index].price,
      };
...
const GlobalContextFunction = (props) => {
  const [{ ProductsAvailable, Cart, CountCart }, dispatch] = useReducer(
    reducerProducts,
    {
      ProductsAvailable: Products.map((t) => {
  ...
  return (
    <GlobalContext.Provider
      value={{ ProductsAvailable, Cart, CountCart, dispatch }}
    >
      {props.children}
    </GlobalContext.Provider>
  );
